%% rateCalc simply calculates the rates for each movement in a lattice
% with applied Hm gradient, gradientHm
load('NNArrays.mat')
load('latticeCoords.mat')

%Calculate x-displacement for each jump
dxANN = a0/24*(vectorsANN(:,:,1)+(0.5*vectorsANN(:,:,2))); %dx = a0*(da+(0.5*db), where da&db scale from -1 to 1. We are on scale 24.
dxINN = a0/24*(vectorsINN(:,:,1)+(0.5*vectorsINN(:,:,2)));
%Calculate temperature difference dT across each jump
dHmANN = gradientHm * dxANN;
dHmINN = gradientHm * dxINN;
...
nuINN = [23.32e12 23.32e12 23.32e12 23.32e12 23.32e12 23.32e12 23.32e12 23.32e12;...
    ...
    36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12;...
    36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12;...
    36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12;...
    36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12 36.87e12];
...
%dHm across the jump distance
ratesANN = nuANN.*exp((-(HmANN+dHmANN))*ec/kbT);
ratesINN = nuINN.*exp((-(HmINN+dHmINN))*ec/kbT);
